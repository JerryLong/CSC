...
public class EaseChatFragment extends EaseBaseFragment implements EMEventListener {
  ...
  //  protected String toChatNickname;
    ...
    //    toChatNickname = fragmentArgs.getString(EaseConstant.EXTRA_NICK_NAME);
    ...
    protected void setUpView() {
    	titleBar.setBackgroundColor(Color.parseColor("#FB5444"));
        titleBar.setTitle(toChatUsername);
        if (chatType == EaseConstant.CHATTYPE_SINGLE) { // 单聊
            // 设置标题
        	EaseUser user = EaseUserUtils.getUserInfo(toChatUsername);
        	System.out.println("user.getNick()==="+user.getNick());
            if((user != null) && (!"".equals(user.getNick()))){
    ...
    protected void sendMessage(EMMessage message){
        if(chatFragmentListener != null){
            ...
            message.setAttribute("Nickname", myNickname);
            message.setAttribute("Avatar", myAvatar);
        }
