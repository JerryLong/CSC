...
import com.yixia.camera.MediaRecorderBase.OnEncodeListener;
...
public class MediaRecordActivity extends MediaBaseActivity
		implements OnErrorListener, OnClickListener, OnPreparedListener, OnEncodeListener {
	...
	/** 回删按钮、延时按钮、滤镜按钮 ,下一步 */
	private CheckedTextView mRecordDelete, mRecordFinish;
	/** 闪光灯 */
	private CheckBox mRecordLed;
	/** 拍摄按钮 */
	private CircleImageView mRecordController;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (DeviceUtils.isZte()) {
			if (dcim.exists()) {
				VCamera.setVideoCachePath(dcim + "/CaiSiChuan/");
			} else {
				VCamera.setVideoCachePath(dcim.getPath().replace("/sdcard/", "/sdcard-ext/") + "/CaiSiChuan/");
			}
		} else {
			VCamera.setVideoCachePath(dcim + "/CaiSiChuan/");
		}
	...
	private void loadViews() {
		...
		mRecordFinish = (CheckedTextView) findViewById(R.id.record_finish);
		mRecordController = (CircleImageView) findViewById(R.id.record_controller);
		...
		mRecordFinish.setOnClickListener(this);
	...
	public void onEncodeComplete() {
		hideProgress();
		mVideoPath = mMediaObject.getOutputTempVideoPath();
	...
	private int checkStatus() {
		...
		if (!isFinishing() && mMediaObject != null) {
			...
			if (duration < RECORD_TIME_MIN) {
				...
				// 视频必须大于3秒
				if (mRecordFinish.getVisibility() != View.INVISIBLE)
					mRecordFinish.setVisibility(View.INVISIBLE);
			} else {
				// 下一步
				if (mRecordFinish.getVisibility() != View.VISIBLE) {
					mRecordFinish.setVisibility(View.VISIBLE);
				}
			}
		}
		return duration;
