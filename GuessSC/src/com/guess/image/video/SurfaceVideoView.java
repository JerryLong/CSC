...
public class SurfaceVideoView extends SurfaceView implements Callback {
	 ...
	 * ()方法。此时可以调用start()方法重新从头播放文件，也可以stop()停止MediaPlayer，或者也可以seekTo()
	 * 来重新定位播放位置。
	 */
	...
	public void setVolume(float volume) {
		...
		// PlaybackCompleted}
		if (mMediaPlayer != null && (mCurrentState == STATE_PREPARED || mCurrentState == STATE_PLAYING
	...
	public boolean isRelease() {
		return mMediaPlayer == null || mCurrentState == STATE_IDLE || mCurrentState == STATE_ERROR
				|| mCurrentState == STATE_RELEASED;
	}
	...
	public void loopDelayed(int startTime, int endTime) {
		...
		mVideoHandler.sendMessageDelayed(
				mVideoHandler.obtainMessage(HANDLER_MESSAGE_LOOP, getCurrentPosition(), delayMillis), delayMillis);
	}
